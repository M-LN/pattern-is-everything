...
(function () {
  ...
  /* Global topic index (all 249+ topics across collections), fetched lazily
     the first time the palette opens. Generated by scripts/build.mjs. */
  var TOPIC_INDEX = [];
  var topicIndexState = 'idle'; // idle | loading | ready | failed
  function loadTopicIndex() {
    if (topicIndexState !== 'idle') return;
    topicIndexState = 'loading';
    fetch('/search-index.json')
      .then(function (r) { return r.ok ? r.json() : Promise.reject(r.status); })
      .then(function (data) {
        if (Array.isArray(data)) { TOPIC_INDEX = data; topicIndexState = 'ready'; }
        else topicIndexState = 'failed';
        // Refresh results if the palette is open with a query typed
  ...
  function buildPalette() {
    ...
    list.addEventListener('click', function (e) {
      ...
      pushRecent(href);
      closePalette();
      // Topic deep-link on the current SPA page: switch topics without a reload
      var hashIdx = href.indexOf('#');
      if (hashIdx > 0) {
        var linkPath = href.slice(0, hashIdx);
        var herePath = window.location.pathname.replace(/index\.html$/, '');
